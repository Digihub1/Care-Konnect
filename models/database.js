...
// Always create Sequelize instance - don't throw errors on configuration
...
// Non-blocking connection test (do not block app startup)
...
if (process.env.DATABASE_URL || (process.env.DB_HOST && process.env.DB_USER && process.env.DB_NAME)) {
    sequelize.authenticate()
        ...
        .catch(err => {
            dbConnected = false;
            console.error('⚠️  Database connection pending:', err.message);
            console.error('📝 Please ensure DATABASE_URL is set in environment variables.');
        });
} else {
    console.warn('⚠️  No database configuration found. Set DATABASE_URL to enable database features.');
}
